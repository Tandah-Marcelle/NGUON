...
const MediaManagement = () => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast({
                title: "Succès",
                description: t('admin.media.toasts.delete_success'),
            });
        } catch (error) {
            console.error('Failed to delete media:', error);
            toast({
                title: t('admin.media.toasts.delete_error'),
                description: t('admin.media.toasts.delete_error'),
                variant: "destructive",
            });
        }
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="font-display text-3xl font-bold text-slate-800 dark:text-white mb-2">{t('admin.media.title')}</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-body">{t('admin.media.description')}</p>
                </div>
                <button
            ...
            <div className="bg-white dark:bg-card rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden font-body">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-white/5 border-b border-slate-100 dark:border-white/10">
                        <tr>
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">{t('admin.media.table.media')}</th>
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">{t('admin.media.table.type')}</th>
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">{t('admin.media.table.date')}</th>
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400">{t('admin.media.table.status')}</th>
                            <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-slate-400 text-right">{t('admin.media.table.actions')}</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={5} className="px-6 py-8 text-center text-slate-400">{t('admin.media.empty')}</td>
                            </tr>
                        ) : (
                            mediaItems.map((item) => (
                                    ...
                                    <td className="px-6 py-4">
                                        {item.published ? (
                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/10 text-green-500 rounded-full text-[10px] font-bold">
                                                <CheckCircle2 size={12} /> {t('admin.media.status.published')}
                                            </span>
                                        ) : (
                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-white/10 text-slate-400 rounded-full text-[10px] font-bold">
                                                <XCircle size={12} /> {t('admin.media.status.draft')}
                                            </span>
                                        )}
                                    </td>
                    ...
                    <div className="bg-white dark:bg-card rounded-3xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col md:flex-row" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-1 bg-slate-100 dark:bg-slate-900 flex items-center justify-center p-8">
                            {previewUrl ? (
                                ...
                                <div className="text-slate-400 text-center">
                                    <p>{t('admin.media.preview.empty')}</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="w-full md:w-96 p-8 space-y-6 overflow-y-auto">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1 block">{t('admin.media.preview.type')}</label>
                                    <p className="text-slate-800 dark:text-white">{previewItem.type}</p>
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1 block">{t('admin.media.preview.url')}</label>
                                    <p className="text-slate-800 dark:text-white text-sm break-all">{previewItem.url}</p>
                                </div>

                                {previewItem.description && (
                                    <div>
                                        <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1 block">{t('admin.media.preview.description')}</label>
                                        <p className="text-slate-800 dark:text-white">{previewItem.description}</p>
                                    </div>
                                )}

                                <div>
                                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1 block">{t('admin.media.preview.status')}</label>
                                    {previewItem.published ? (
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-500/10 text-green-500 rounded-full text-xs font-bold">
                                            <CheckCircle2 size={14} /> {t('admin.media.status.published')}
                                        </span>
                                    ) : (
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-white/10 text-slate-400 rounded-full text-xs font-bold">
                                            <XCircle size={14} /> {t('admin.media.status.draft')}
                                        </span>
                                    )}
                                </div>

                                <div>
                                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1 block">{t('admin.media.preview.created_at')}</label>
                            ...
                            {previewUrl && (
                                ...
                                >
                                    <Download size={20} />
                                    {t('admin.media.preview.download')}
                                </button>
                            )}
                        </div>
                    </div>
            ...
            {deleteItem && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setDeleteItem(null)}>
                    <div className="bg-white dark:bg-card rounded-3xl shadow-2xl max-w-md w-full p-8" onClick={(e) => e.stopPropagation()}>
                        <h2 className="font-display text-2xl font-bold text-slate-800 dark:text-white mb-4">{t('admin.media.delete_modal.title')}</h2>
                        <p className="text-slate-600 dark:text-slate-400 mb-6">
                            {t('admin.media.delete_modal.description')} <span className="font-bold">{deleteItem.title}</span> ? {t('admin.media.delete_modal.warning')}
                        </p>
